...
def sample(sample_num, dqn_model):    
    ...
    value_sampled_DQN = [] # Stores the value sampled from the pretrained DQN model
    value_sampled_random = [] # Stores the value sampled randomly
    ...
    for i in range(len(dataset)//N_options):
        ...
        value_sampled_DQN.append(state_value_list[choice])
        value_sampled_random.append(state_value_list[choice_random])

    with open('sampled_data/value_sampled_' + str(dqn_model) + '_' + str(sample_num), 'wb') as handle:
        pickle.dump(value_sampled_DQN, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open('sampled_data/value_sampled_random_' + str(dqn_model) + '_' + str(sample_num), 'wb') as handle:
        pickle.dump(value_sampled_random, handle, protocol=pickle.HIGHEST_PROTOCOL)
